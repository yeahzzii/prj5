...
<body>
<img class="banner" src="a00_com\images\banner.png">
<img class="CO2" src="a00_com\images\CO2.JPG">
<nav role="navigation">
  <ul id="main-menu">
   	...
   	<form id="frm01"
      	...
      	<div class="input-group mb-3">
         	<div class="input-group-prepend">
            	<span class="text-center input-group-text">ID</span>
         	</div>
         	<input name="id" class="form-control" size="20"
            	value="${members.id }" readonly/>    
      	</div>
      	<div class="input-group mb-3">
         	<div class="input-group-prepend">
            	<span class="text-center input-group-text">PW</span>
         	</div>
         	<input type="password" name="pw" class="form-control" size="20"
            	value="${members.pw}" placeholder="비밀번호를 입력하세요." />
      	</div>
      	<div class="input-group mb-3">
         	<div class="input-group-prepend">
            	<span class="text-center input-group-text">PW Confirm</span>
         	</div>
         	<input type="password" name="pwConfirm" class="form-control" size="20"
            	value="" placeholder="위 비밀번호와 똑같이 입력하세요." />
		</div>
		<div class="input-group mb-3">
			<div class="input-group-prepend">
				<span class="text-center input-group-text">PHONE</span>
			</div>
			<input type="tel" name="phonenum" value="${members.phonenum }" size="20"
				placeholder="010-****-**** (- 포함)" pattern="[0-9]{3}-[0-9]{4}-[0-9]{4}"/>
		</div>
		<div class="input-group mb-3">
      		<div class="input-group-prepend">
				<span class="text-center input-group-text">BIRTH</span>
			</div>
			<input type="date" name="birth" value="${members.birth }" size="20">
      	</div>
      	<div class="input-group mb-3">
      		<div class="input-group-prepend">
				<span class="text-center input-group-text">E-MAIL</span>
			</div>
			<input type="email" name="email" value="${members.email }" size="20">
      	</div>
...
function updateProc(){
	if(confirm("수정하시겠습니까?")){
		// 유효성 check
		$("form").attr("action", "${path}/updateMembersInfo.do");
		$("form").submit();
	}
}
function deleteProc(){
	if(confirm("삭제하시겠습니까?")){
		$("form").attr("action", "${path}/deleteMembersInfo.do");
		$("form").submit();
	}
}
var proc = "${proc}"
